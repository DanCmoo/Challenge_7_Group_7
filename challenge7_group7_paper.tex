...
(i)~training from random initialisation, (ii)~frozen-backbone feature extraction, and
...
synthesise Clipart-style training images from Art-domain content, generating a 180-image
augmentation corpus.
Third, we assess two domain adaptation strategies---target-domain fine-tuning on 40~labelled
...
Grad-CAM visualisations and t-SNE projections before and after adaptation illuminate the
...
paintings (\textbf{Art}), flat-colour \textbf{Clipart}, product photographs (\textbf{Product}),
...
\begin{enumerate}
  \item \textbf{Part A — Few-shot classification}: How much do pretraining and fine-tuning
        strategy matter when the labelled source-domain budget is tiny
        (18--51 images per class)?
  \item \textbf{Part B — Neural Style Transfer}: Can we synthesise plausible Clipart-style
        images from Art content using Gatys et al.'s optimisation-based NST, and how
        do hyperparameter choices (content/style weight ratio $\alpha/\beta$) affect
        the generated images?
  \item \textbf{Part C — Domain adaptation}: Does fine-tuning on a small labelled
...
ImageNet classification) to improve generalisation on a smaller target task~\cite{tan2018transfer}.
...
$\mathbf{x}$ is obtained by minimising:
...
Optimisation is performed with L-BFGS directly on the pixel values of $\mathbf{x}$.
...
The simplest supervised approach is to fine-tune a pretrained model on a small labelled
target set.
...
Table~\ref{tab:dataset} summarises the per-class image counts for the two domains used.
...
All images are resized to $224\times224$ pixels and normalised with ImageNet
mean and standard deviation.
Training augmentation consists of random horizontal flipping and mild
colour jitter; evaluation uses centre crop only.
...
\begin{enumerate}
  \item \textbf{From scratch}: all weights initialised randomly; serves as the lower-bound baseline.
...
\begin{table}[h]
\centering
\caption{Part C Results — All five model variants on Clipart test (target).}
\label{tab:partc}
\begin{tabular}{lrrr}
\toprule
\textbf{Strategy} & \textbf{Src Acc} & \textbf{Tgt Acc} & \textbf{$\Delta_{\text{shift}}$} \\
...
Yes.
Even the frozen-backbone strategy substantially outperforms training from scratch,
confirming that ImageNet features provide a strong generic visual representation~\cite{kornblith2019transfer}.
The gain from fine-tuning over a frozen backbone depends on whether the source domain
is large enough to prevent overfitting of the unfrozen layers; with only 126 Art
training images (70\,\% of 180), early stopping is essential.

\subsection{What does the $\alpha/\beta$ ratio reveal about the Art\,$\to$\,Clipart gap?}

The Clipart domain is characterised by flat colours, sharp edges, and absence of
...
in the Art domain.
The ratio $10^{-4}$ constitutes a sweet spot where Clipart colour statistics are adopted
...
Target fine-tuning reduces $\Delta_{\text{shift}}$ more than style augmentation.
The Art\,$\to$\,Clipart shift is dominated by colour distribution and line-art appearance,
which 40 real Clipart examples per class are sufficient to capture at the feature level.
Style augmentation is label-free and provides additional training signal, but
the NST procedure generates painterly artefacts rather than authentic vector-art
graphics, limiting its fidelity to the true target distribution.
Nevertheless, style augmentation offers a meaningful improvement over the unadapted
baseline at zero target-label cost.
...
Clipart domain (e.g., Mouse vs.\ Keyboard — both small, dark objects on white
...
\begin{enumerate}
  \item \textbf{ImageNet pretraining dominates data size.}
        The frozen-backbone model substantially outperforms random initialisation
  ...
  \item \textbf{NST-based augmentation is a viable unsupervised bridge.}
        ...
        baseline, at no label cost, though quality is limited by the optimisation-based
        generation approach.
  \item \textbf{Labelled target fine-tuning achieves the strongest adaptation.}
        Even a small labelled target set (40 images per class) provides a more accurate
